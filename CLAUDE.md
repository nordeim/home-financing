...
- **Playwright E2E** — `@playwright/test 1.63.0` + `@axe-core/playwright 4.13.0`, `playwright.config.ts` (prod `next start` on 3002, `reuseExistingServer:true`), **61 tests per project** (47 declarations expanding via data-driven loops: +14 asset URLs, +2 H4 articles): `smoke.spec.ts` (7 — home/nav/footer, get-started via the "Get Started" CTA, calculator, health, 404, axe critical), `seo.spec.ts` (5 — sitemap absolute locs + host-rewrite, robots, title, OG), `funnel.spec.ts` (4 — POST `/api/applications` 400/200 + `x-forwarded-for` isolated, burst 429, UI no-500), `assets.spec.ts` (19 runtime — 14 referenced image assets return 200 + no broken `<img>` on `/`, `/adu-financing`, `/tiny-home-financing`, and the two `/compare/*` alias redirects resolve), `parity.spec.ts` (26 runtime — markdown-OOM regression on the two H4 articles, wordmark logos + testimonial avatars, learn-hub search/filter/featured/tools, calculator breakdown/PMI, footer Socials/Legal, wizard, plus the pass-3 live-source pins: circle brand mark + two-tone wordmark, always-light header (desktop + mobile), no intro eyebrow (source renders none — corrected in pass 4), closing trust line, no green band, single steps CTA, title-cased eyebrow, 2-item More dropdown, footer copyright + legal-line NMLS link, get-started 3-step band, calculator amber band; and the pass-4 pins: source hero overlay recipe + bottom fade + no hero-grid, hub Last-Updated line + truth callout + dual CTA + 4 chips, FHA/VA highlight + dual CTA + author strip, learn no-"read read", calculator Free-Calculator pill + middle crumb). With Postgres: 61/61; without DB: 60/61 (funnel valid-payload is the only DB-dependent test).
...
npm run e2e         # playwright (chromium, 61 tests per project)
```
...
- **Header:** fixed `h-16`; **always light** — `bg-background/90` + `backdrop-blur-xl` (source-measured 16px) + border in every state (homepage hero included). Live modfii.com never renders a transparent header (computed-style probe 2026-09-12: `rgba(253,253,252,0.8)` + blur at top, scrolled, and mobile — the earlier transparent-over-dark treatment was removed in pass 3). CTA is a forest "Get Started" pill (never amber). Desktop More dropdown = exactly two items (ADU Financing, Tiny Home Financing); the mobile drawer keeps NAV + MORE.
...
- **Homepage (`src/app/page.tsx`)** mirrors modfii.com section-for-section: photo hero + glass stats card (source recipe: daylight `hero-prefab.jpg` under `from-primary/95 via-primary/90 to-primary/80` + bottom fade `to-primary/95`, content-driven height, `text-4xl→6xl` headline — pass 4), manufacturer wordmark strip (real logo PNGs in `public/brand/wordmarks/`; skyline swapped to the source-inlined asset in pass 4), "Modular & Prefab Home Loans" intro (no eyebrow — the source renders none, 4 cards + loan chips, scroll-revealed via `Reveal`), problem/solution (red vs green tinted cards with icons) + "See Your Options" CTA, 3-step icons + ghost numerals (no section CTA — source has none), 5-star testimonials with portrait avatars (`public/images/avatars/`) + savings ledger (role and location on separate lines like the source), "Our Standards" trio (ShieldCheck/BookOpen/Users icons), animated FAQ accordions, dotted-pattern closing CTA with the shield-icon "No credit impact • 15-minute application • Cancel anytime" trust line. The pass-2 "green sustainability band" was removed in pass 3 — the live source homepage has no such section.
...
- **Layout rhythm:** `Container` max `1400px`, `px-4 md:px-8`, header `h-16` fixed + `backdrop-blur-xl` + `border-b`.
...
*Updated 2026-09-13 (pass 4): error boundary `retry` doc retained; source-exact home hero (daylight photo + dual-overlay recipe + content height + text-6xl cap), intro eyebrow removed (source renders none — pass-3 pin flipped), hub Last-Updated + truth callout + dual CTA + 4th chip + crumbsOutside, FHA/VA/USDA/construction source heroes + author strips + contextual eyebrow icons + amber first CTA, learn read-duplication fix, calculator Free-Calculator pill + middle crumb + `--color-chart-tax` blue tax segment, header blur 12→16px, hero photo + skyline wordmark swapped to source assets; counts now 37 unit + 61 E2E per project (smoke 7 + seo 5 + funnel 4 + assets 19 + parity 26; 47 declarations + data-driven loops); `turbo.json` claims fixed across docs; full gate `lint → typecheck → test → build → e2e` green (60/61 DB-less; funnel-persist needs Postgres). Evidence: `docs/REMEDIATION_PLAN_pass4.md`. Earlier updates: 2026-09-13 pass-3 (`retry` prop), 2026-09-12 pass-3 live-source parity, 2026-09-11 passes 1–2.*
