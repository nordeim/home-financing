...
drizzle/
  ...
  meta/_journal.json
 e2e/
  smoke.spec.ts / seo.spec.ts / funnel.spec.ts /  # Playwright 81 tests (chromium)
...
npm run e2e         # playwright (chromium, 82 tests per project)
```
...
- **Header:** fixed, inner bar `h-20`; **always light** — `bg-background/80` + `backdrop-blur-lg` (16px) + `border-border/50` in every state (homepage hero included) — aligned to the live source bar in pass 5 (2026-09-12: source probes `rgba(253,253,252,0.8)` + blur(16px) + 80px inner + 1px border). CTA is a forest "Get Started" pill (never amber). Desktop More dropdown = exactly two items (ADU Financing, Tiny Home Financing); the mobile drawer keeps NAV + MORE.
...
- **Layout rhythm:** `Container` max `1400px`, `px-4 md:px-8`, header `h-20` fixed + `backdrop-blur-lg` + `border-b border-border/50`. Radius tokens match the source scale (pass-5): `--radius-md: 0.625rem` (10px buttons), `--radius-2xl: 1rem` (16px cards). Button base is `font-medium` with lg `px-8`; `lgSm` (text-sm lg) and `xl` (text-lg) sizes mirror the source CTA mix.
...
- **Unit tests:** vitest is installed — `src/lib/*.test.ts` co-located, `npm run test` (37 tests: calculator 11 + matching 13 + rate-limit 9 + markdown 8). Keep unit tests on pure domains; DB paths belong to Playwright E2E.
