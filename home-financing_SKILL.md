...
e2e/ (61 per project — 53 declarations + data-driven loops, `parity.spec.ts` pins H4/OOM + parity + pass-3 live-source pins + pass-4 source-exact hero/guide-hero pins)
├── smoke.spec.ts (7)
├── seo.spec.ts (6)
├── funnel.spec.ts (4)
...
npm run e2e             # chromium 82/82 per project (53 declarations + data-driven loops; 81/82 DB-less)
...
| Pre-ship gate | `AGENTS.md` | `db:setup → lint (0/0) → typecheck → test 41/41 → build 43/43 → e2e 82/82` | `drizzle` journal `idx 0,1` |
...
- [ ] `npm run test 2>&1 | grep Tests` → `41` (11+13+9+8) still; `npm run e2e 2>&1 | grep "passed"` → `81` per project (81/82 DB-less — funnel valid-payload needs Postgres)
...
*Last verified 2026-09-12 (SKILL v1.3.1 — pass-5 pre-remediation alignment audit in a clean clone: `lint 0/0`, `typecheck` clean, `41/41` unit, `build 43/43` (routes `○ 42 static + ƒ 8 dynamic` — includes `/debug-error-probe` probe route + `/editorial-policy` + `/corrections`), `82/82` E2E chromium with Postgres + `81/82` DB-less; validation-checklist drift fixed — `5/9` client islands (not `3/7`), e2e count `61` (not `44`/`55`); live-audit https://modfii.jesspete.shop → 39 PASS / 0 FAIL / 2 WARN; source-parity drift catalogued for remediation pass 5 (header bar metrics, intro-eyebrow re-add — pass-4 removal was based on a wrong pin, guide-page content depth, button chrome, heading levels). Earlier: 2026-09-13 pass 4 — source-exact home hero, hub Last-Updated + truth callout, loan-page heroes, calculator pill + blue tax segment; `41/41` vitest = 11+13+7+6 incl. H4/OOM + `82/82` per project playwright = smoke 8+seo 5+funnel 5+assets 19+parity 45 (81/82 DB-less) + DB init `8/40/50/23/59/5`.) against `package.json` (next ^16.3.4), `tsconfig.json` (strict, skills excluded), `next.config.ts` (11 redirects), `drizzle.config.*` (`5434`), `docker-compose.yml` (`home_financing_*` + `pgcrypto/pg_trgm`), `src/db/schema.ts` (8 tables), `src/lib/*` (37 tests), `public/brand/wordmarks 5` + `public/images/avatars 3`, `playwright.config.ts` (`3002`), `e2e/*` 61 per project, `.env.example`. Evidence: `docs/REMEDIATION_PLAN_pass3.md` + `docs/REMEDIATION_PLAN_pass4.md` + `docs/REMEDIATION_PLAN_pass5.md`.*
