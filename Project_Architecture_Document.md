...
**Last Updated:** 2026-09-12 (v1.5 — pass 6 audit remediation: rate-limit sweep, SAFE_HREF, exclusive accordions; counts 41 unit / 82 E2E; audit GO. Pass 5: security headers app-emitted, funnel JSON-500 contract, source-exact header/logo/buttons/radius, heading hierarchy + intro eyebrow restored, guide content depth, counts 37 unit / 82 E2E per project)
...
| Unit Testing | Vitest | `^3.2.7` (`vitest/config`, `environment: node`) | Co-located `src/lib/*.test.ts` on pure domains only (calculator 11 + matching 13 + rate-limit 9 + markdown 8 = 37); DB paths stay under Playwright. `include: src/**/*.test.ts`. |
| E2E Testing | Playwright + `@axe-core/playwright` | `^1.63.0` + `^4.13.0` | Prod `npx next start --port 3002` (not `dev`), `reuseExistingServer:true`, `timeout: 30000/5000`, projects `chromium + webkit`, `x-forwarded-for` isolation for rate-limit tests. 81 per project — 53 declarations + data-driven loops (162 with webkit, 82/82 with DB, 81/82 DB-less). |
...
#### ADR-006: Vitest Unit (37) + Playwright Prod-Build E2E (55) — Pure vs. DB Boundary
...
- **Decision:** Vitest `3.2.7` (node env, `@` alias, `include: src/**/*.test.ts`) for `src/lib/{calculator,matching,rate-limit,markdown}.test.ts` — **37 tests** (calculator 11 + matching 13 + rate-limit 9 + markdown 8) on pure domains only — markdown pins the 2026-09-11 H4/OOM regression (both killing articles). Playwright `1.63.0` + `@axe-core/playwright 4.13.0` for `e2e/{smoke,seo,funnel,assets,parity}.spec.ts` — **81 per project** (53 declarations + data-driven loops: +14 asset URLs, +2 H4 articles; 162 with webkit) running `npx next start --port 3002` (not `dev`), `reuseExistingServer:true`, `expect:5000`. `assets.spec.ts` pins the two `2026-09-11` incidents: 14 image assets `200` (6 heroes/OG + 5 wordmarks + 3 avatars) + 3 broken-img checks + 2 `/compare/*` parity aliases resolve. `parity.spec.ts` pins H4 OOM + wordmarks/avatars/learn-hub/calculator/footer/wizard + the pass-3 live-source pins + the pass-4 source-exact pins (hero overlay recipe/bottom fade/no hero-grid, hub Last-Updated + truth callout + dual CTA + 4 chips, FHA/VA highlight + dual CTA + author strip, learn no-"read read", calculator pill + middle crumb). Funnel valid-payload E2E needs Postgres (migrated+seeded); the other 80 run DB-less.
- **Rationale:** `lint (0/0)` + `typecheck` + `test (41/41)` is the fast local gate; `build (43/43)` validates redirects + RSC boundaries + `skills` exclusion; `e2e 82/82` on the shipped artifact catches what `dev` HMR hides (assets 19 runtime + parity 45 runtime incl. the pass-3 + pass-4 live-source pins).
...
| E2E — SEO | Playwright | 1 | 5 | `e2e/seo.spec.ts` | Optional | (same) |
...
| E2E — parity | Playwright | 1 | 26 (25 declarations + 2 H4-article loop) | `e2e/parity.spec.ts` | **Required** for H4 OOM / wordmarks+avatars/learn-hub/calculator/footer/wizard (other 60 DB-less) | (same) |
| **Total** | Vitest + Playwright | 8 | **98 (41+82)** | `src/lib/*.test.ts` + `e2e/*.spec.ts` | See notes | `npm run db:setup → lint → typecheck → test → build → e2e` |

With Postgres: **82/82** E2E (chromium, 53 declarations + data-driven loops). Without DB: **81/82** (funnel valid-payload is the only DB-dependent test — expects `200` with `application_matches` persistence; other 60 verify static/SEO/assets/parity). `e2e:all` runs `chromium + webkit` (164 total).
...
| E2E | `npm run e2e` (prod `next start` on `3002`) + `npm run e2e:all` (chromium+webkit) | `82/82` (with DB, per project) / `81/82` (without DB, funnel valid-payload blocked) |
...
No `CRITICAL` open issues. `lint 0/0`, `typecheck` pass, `41/41` unit (11+13+9+8), `82/82` E2E per project (81/82 DB-less), `build 43/43`, live probes green as of `2026-09-13` (pass 4: source-exact home hero, hub truth callout + Last-Updated, loan-page source heroes + author strips, learn read-duplication fix, calculator pill + blue tax segment, intro-eyebrow removal — see `docs/REMEDIATION_PLAN_pass4.md`).
...
| `e2e/{smoke,seo,funnel,assets,parity}.spec.ts` | 63/77/99/74/147 | 82 E2E tests per project (smoke 8, seo 5, funnel 5, assets 19 runtime, parity 45 runtime) incl. H4-OOM + alias/image regression + pass-3/pass-4 live-source parity guards |
| `src/lib/*.test.ts` | 81/110/57 | 37 Vitest unit tests co-located on pure domains |
...
*Last verified 2026-09-13 (PAD v1.3 — remediation pass 4: source-exact home hero (daylight photo + `from-primary/95 via-primary/90 to-primary/80` + bottom fade + content-driven height + text-4xl→6xl headline, no hero-grid), intro eyebrow REMOVED (live source renders none — pass-3 pin flipped), hub hero Last-Updated line + "Here's the truth" glass callout + dual CTA + 4th chip + crumbsOutside breadcrumbs, FHA/VA/USDA/construction loan pages source copy + amber-first CTA + contextual eyebrow icons + author/reviewer strips + FHA chip removal, learn "read read" fix, calculator Free-Calculator pill + middle crumb + `--color-chart-tax` blue tax segment, header blur 12→16px, hero photo + skyline wordmark swapped to source assets; pass 3 (2026-09-12): circle brand mark + two-tone wordmark, always-light header, closing trust line, green band removed, get-started 3-step band, calculator amber band, footer legal line + 2-item More dropdown, PMI-flake root cause; counts `41/41` vitest = 11+13+7+6 + `82/82` per project playwright = smoke 8+seo 5+funnel 5+assets 19+parity 45 (81/82 DB-less) + `next build` 43/43 + `lint 0/0` + live probes (`/api/health` ok:true, funnel 5 matches, sitemap 152, robots host correct) against `package.json` (next ^16.3.4, react ^19.3.0, tailwind ^4.3.3, vitest ^3.2, playwright 1.63), `tsconfig.json` + `eslint.config.mjs` (skills+infrastructure ignored), `next.config.ts` (11 redirects), `drizzle.config.ts/.json` (5434, strict/verbose), `docker-compose.yml` (home_financing_*), `src/db/schema.ts` (8 tables), `src/lib/*` (37 tests), `public/brand/wordmarks 5` + `public/images/avatars 3` + `public/images 5` + `brand/og-image.jpg`, `playwright.config.ts` (3002), `e2e/*` 55 per project, `vitest` 41/41. Source of truth: `package.json`, `tsconfig.json`, `next.config.ts`, `eslint.config.mjs`, `drizzle.config.ts/.json`, `docker-compose.yml:5434`, `src/db`, `src/lib`, `src/scripts`, `e2e`, `vitest.config.ts`, `.env.example`, live modfii.com + modfii.jesspete.shop probes. Evidence: `docs/REMEDIATION_PLAN_pass4.md` (pass 3: `docs/REMEDIATION_PLAN_pass3.md`).*
