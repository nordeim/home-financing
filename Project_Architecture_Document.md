...
**Last Updated:** 2026-09-15 (v1.6 — pass 9 drift alignment D-01→D-07: STATIC_PATHS 40→39 (152 total = 39+23+50+40), build 36+7/42+8→35+8 (43 routes), E2E 82→121, vitest badge 3.2→4.1, playwright 122 listed→121 executed; validation `docs/VALIDATION_REPORT_2026-09-15.md` 121/121 GO. v1.5 — pass 6 audit remediation: rate-limit sweep, SAFE_HREF, exclusive accordions; counts 41 unit / 121 E2E (smoke 8 + seo 16 + funnel 5 + assets 19 + parity 73; 103 decl); audit GO. Pass 5: security headers app-emitted, funnel JSON-500 contract, source-exact header/logo/buttons/radius, heading hierarchy + intro eyebrow restored, guide content depth, counts 41 unit / 121 E2E per project)
...
- `[CA, build]` Next build `43/43` (`○ 35 static + ƒ 8 dynamic`) with 11 redirects validated; `images.unoptimized:true` locked. — *2026-09-15 correction (D-02): earlier `36+7` and `42+8` shorthands were arithmetic drift; observed `35 static + 8 dynamic = 43` (ƒ: `api/health`, `api/applications`, `api/calculator`, `authors/[authorSlug]`, `debug-error-probe`, `learn/[slug]`, `manufacturers/[slug]`, `states/[state]`).*
- `[CA, 2026-09-12 v1.2 remediation pass 3]` Live-source parity against rendered `modfii.com` (DOM + computed-style probes, 1440px + 390px): brand mark → circle glyph + two-tone `Mod|Fii` wordmark; header → always-light frosted bar (`rgba(253,253,252,0.8)` + blur at top/scrolled/mobile — transparent-over-dark removed, `overDarkHero`/scroll-listener deleted); homepage + "Your Prefab Financing Partner" eyebrow + closing trust line, − green sustainability band, − duplicate steps CTA, `How it works` → `How It Works`; `/get-started` + "Get Financing in 3 Easy Steps" band; `/calculator` lower sections on `bg-accent/15`; footer copyright − `NMLS #`, legal line + NMLS Consumer Access link; More dropdown 4→2 items. E2E `44→55` per project (parity `8→19`; 42 declarations + 14 asset variants; 92 total with unit). PMI interaction test flake root-caused (pre-hydration dispatch poisons React's value tracker; identical re-dispatches swallowed) → fresh-navigation `toPass` retry, ×10 stable. Verified `lint 0/0` + `typecheck` + `41/41` + `build 43/43` + `54/55` chromium DB-less (funnel valid-payload needs PG). Evidence: `docs/REMEDIATION_PLAN_pass3.md`.
...
| Unit Testing | Vitest | `^4.1.11` (`vitest/config`, `environment: node`) | Co-located `src/lib/*.test.ts` on pure domains only (calculator 11 + matching 13 + rate-limit 9 + markdown 8 = 41); DB paths stay under Playwright. `include: src/**/*.test.ts`. |
| E2E Testing | Playwright + `@axe-core/playwright` | `^1.63.0` + `^4.13.0` | Prod `npx next start --port 3002` (not `dev`), `reuseExistingServer:true`, `timeout: 30000/5000`, projects `chromium + webkit`, `x-forwarded-for` isolation for rate-limit tests. 121 per project — 103 declarations + data-driven loops (242 with webkit, 121/121 with DB, 120/121 DB-less; `playwright --list` shows 122 listed but 121 executed). |
...
- **Rationale:** RSC streaming + `next/font` variable fonts + `next/image` `priority` even when unoptimized; `export const dynamic = "force-dynamic"` scoped to DB-touching routes (`/api/*`) while content pages remain static-prerenderable (build `○ 35 static + ƒ 8 dynamic`). `redirects()` validated at build (11 permanent/temporary) prevents 404 after slug renames — pinned by `assets.spec.ts`.
- **Consequences:** Positive — SEO parity with `modfii.com`, correct RSC boundaries caught by `next build`. Negative — must not import Server Component into Client Component (breaks build); global `src/app/{loading,error}.tsx` now cover dynamic segments (8 dynamic routes) as fallback — per-segment `loading.tsx`/`error.tsx` remain incremental.
...
#### ADR-006: Vitest Unit (41) + Playwright Prod-Build E2E (121) — Pure vs. DB Boundary
...
- **Decision:** Vitest `4.1.11` (node env, `@` alias, `include: src/**/*.test.ts`) for `src/lib/{calculator,matching,rate-limit,markdown}.test.ts` — **41 tests** (calculator 11 + matching 13 + rate-limit 9 + markdown 8) on pure domains only — markdown pins the 2026-09-11 H4/OOM regression (both killing articles). Playwright `1.63.0` + `@axe-core/playwright 4.13.0` for `e2e/{smoke,seo,funnel,assets,parity}.spec.ts` — **121 per project** (103 declarations + data-driven loops: smoke 8 + seo 16 + funnel 5 + assets 19 + parity 73; `playwright --list` shows 122 listed but 121 executed — 122 is a display artifact counting one alias helper; 242 with webkit) running `npx next start --port 3002` (not `dev`), `reuseExistingServer:true`, `expect:5000`. `assets.spec.ts` pins the two `2026-09-11` incidents: 14 image assets `200` (6 heroes/OG + 5 wordmarks + 3 avatars) + 3 broken-img checks + 2 `/compare/*` parity aliases resolve. `parity.spec.ts` pins H4 OOM + wordmarks/avatars/learn-hub/calculator/footer/wizard + the pass-3 live-source pins + the pass-4 source-exact pins (hero overlay recipe/bottom fade/no hero-grid, hub Last-Updated + truth callout + dual CTA + 4 chips, FHA/VA highlight + dual CTA + author strip, learn no-"read read", calculator pill + middle crumb). Funnel valid-payload E2E needs Postgres (migrated+seeded); the other 120 run DB-less (121/121 with DB).
- **Rationale:** `lint (0/0)` + `typecheck` + `test (41/41)` is the fast local gate; `build (43/43)` validates redirects + RSC boundaries + `skills` exclusion; `e2e 121/121` on the shipped artifact catches what `dev` HMR hides (assets 19 runtime + parity 73 runtime incl. the pass-5/7 live-source pins); `120/121` DB-less.
...
- **Decision:** `next.config.ts:images.unoptimized:true` intentional — `next/image` still provides `priority`, sizing, and a11y, but serves images unoptimized. Retaining `sharp` would require infra change. `DISABLE_IMAGE_OPTIMIZER=1` has no `turbo.json` to live in (this repo is not a Turborepo — resolved in pass 4; wire into CI env if Turborepo is ever added).
...
flowchart TB
  subgraph Client["Client — Browser"]
    ...
    HEADER["SiteHeader<br/>fixed h-16<br/>always-light frosted bar (pass 3)<br/>NAV 4 + MORE 2 dropdown + mobile drawer"]
  end
...
│   │   ├── sitemap.ts                   ← 39 STATIC_PATHS + articles→/learn/[slug] + states→/modular-home-financing/states/[slug] + manufacturers→… (152 locs = 39+23+50+40, absolute via base)
...
│   │   ├── site-header.tsx              ← "use client" — fixed h-16 md:h-20 always-light frosted bar (bg-background/80 backdrop-blur-lg border-border/50), NAV (4) + MORE (2), mobile drawer, useId, adjust-during-render pathname close (no overDarkHero)
...
├── vitest.config.ts                     ← alias @→./src, environment node, include src/**/*.test.ts (calculator 11 + matching 13 + rate-limit 9 + markdown 8 = 37)
...
| Rule | Value | Where |
|------|-------|-------|
| Header transition | Removed in pass 3 — header statically light | `ChevronDown` on `MORE` still `transition rotate-180` |
...
**Header contract — emitted by the app since pass 5 (2026-09-12):** `next.config.ts:headers()` now ships the full set on every route; verified live on the local prod server and pinned by `e2e/smoke.spec.ts` (CSP incl. `frame-ancestors 'none'`, XFO DENY, nosniff, referrer-policy, permissions-policy; HSTS also set for HTTPS deployments):
...
|----------|-----------|-------|-------|----------|----|---------|
| Unit — pure domains | Vitest `4.1.11` (`node`) | 4 | 41 | `src/lib/calculator.test.ts` (11), `matching.test.ts` (13), `rate-limit.test.ts` (9), `markdown.test.ts` (8) | None | `npm run test` / `test:watch` / `test:coverage` |
| E2E — smoke | Playwright `1.63.0` + `@axe-core/playwright 4.13.0` | 1 | 8 | `e2e/smoke.spec.ts` | Optional (`health` tolerates `200|500`) | `npm run e2e` |
| E2E — SEO | Playwright | 1 | 16 | `e2e/seo.spec.ts` | Optional | (same) |
| E2E — funnel | Playwright | 1 | 5 | `e2e/funnel.spec.ts` | **Required** for valid-payload 200 path (other 4 run DB-less) | (same) |
| E2E — assets | Playwright | 1 | 19 (14 looped) | `e2e/assets.spec.ts` | None | (same) |
| E2E — parity | Playwright | 1 | 73 | `e2e/parity.spec.ts` | **Required** for H4 OOM / wordmarks+avatars/learn-hub/calculator/footer/wizard + pass-5/7 live-source pins (other 48 DB-less) | (same) |
| **Total** | Vitest + Playwright | 8 | **162 (41+121)** | `src/lib/*.test.ts` + `e2e/*.spec.ts` | See notes | `npm run db:setup → lint → typecheck → test → build → e2e` |

With Postgres: **121/121** E2E (chromium, 103 declarations + data-driven loops — `playwright --list` shows 122 listed but 121 executed). Without DB: **120/121** (funnel valid-payload is the only DB-dependent test — expects `200` with `application_matches` persistence; other 120 verify static/SEO/assets/parity). `e2e:all` runs `chromium + webkit` (242 total).
...
npm run test          # vitest 41/41 (11+13+7+6, incl. markdown H4/OOM regression)
npm run build         # next build — validates redirects + RSC boundaries + skills excluded
npm run e2e           # playwright chromium 82/82 per project (53 declarations + data-driven loops; valid-payload needs DB)
...
**Build artifact verified 2026-09-12 (pass-5 clean-clone re-run):**

```
Route (app)
┌ ○ /  (+ 41 static: /about, /adu-financing, /calculator, /compare/*, /construction-loans/*, /modular-home-financing/**, /editorial-policy, /corrections, /glossary, /resources, /robots.txt, /sitemap.xml, /terms, /tiny-home-financing)
├ ƒ /api/applications
├ ƒ /api/calculator
├ ƒ /api/health
├ ƒ /authors/[authorSlug]
├ ƒ /debug-error-probe        # deliberate error-boundary probe (error.tsx catches → recovery UI; live deploy returns 200)
├ ƒ /learn/[slug]
├ ƒ /modular-home-financing/manufacturers/[slug]
├ ƒ /modular-home-financing/states/[state]
└ ○ /_not-found

○ (Static)  prerendered as static content — 42 routes
ƒ (Dynamic) server-rendered on demand — 8 routes

✓ Running next.config.ts took 27ms (redirects validated)
...
| `DISABLE_IMAGE_OPTIMIZER` | No | `1` in sandboxes where `sharp` deadlocks; no `turbo.json` in this repo — wire into CI env if Turborepo is added | `1` |
...
| Quality | `npm run lint` (flat + `core-web-vitals`) + `npm run typecheck` + `npm run test` | `lint 0/0`, `typecheck` pass (`skills` excluded), `test 41/41 (11+13+9+8)` |
| Build | `npm run build` | `43/43` pages, redirects validated, RSC boundaries proven |
| E2E | `npm run e2e` (prod `next start` on `3002`) + `npm run e2e:all` (chromium+webkit) | `121/121` (with DB, per project) / `120/121` (without DB, funnel valid-payload blocked) |
...
| `npm run test` | `vitest.config.ts` | `vitest run` 37 tests (11+13+9+8) |
...
| **HIGH** | ~~No security headers emitted by the app itself~~ | Clickjacking, MIME-sniffing, protocol-downgrade exposure | **Resolved 2026-09-12 (pass 5)** — `next.config.ts:headers()` emits CSP/XFO/nosniff/referrer-policy/permissions-policy + HSTS on every route; pinned by `e2e/smoke.spec.ts`. NOTE: the live deploy still needs a redeploy to pick this up |
| **MEDIUM** | ~~`POST /api/applications` empty non-JSON 500 on DB outage~~ | API consumers couldn't distinguish DB outage | **Resolved 2026-09-12 (pass 5)** — persistence block wrapped in try/catch → `Response.json({ error }, { status: 500 })`; pinned by `funnel.spec.ts` + verified via `scripts/verify-db-outage.sh` (500 + `application/json` + error body) |
...
| **MEDIUM** | `README.md` troubleshooting row said "`.env.example` still contains legacy `scandihaven_*` placeholders" — now actually `home_financing_*` | Doc stale sentence could mislead onboarding | **Resolved** — README row documents the canonical `home_financing_*` `:5434` values; `.env.example` matches `docker-compose.yml` |
| **MEDIUM** | `CLAUDE.md` references `turbo.json:globalEnv` for `DISABLE_IMAGE_OPTIMIZER` but no `turbo.json` exists | Minor contract drift | **Resolved 2026-09-13 (pass 4)** — claims reworded across CLAUDE.md/README.md/PAD/.env.example/SKILL to state no `turbo.json` exists; revisit only if Turborepo is introduced |
...
| **MEDIUM** | ~~Rate-limit bucket map grew unboundedly under unique-key floods~~ (audit A-01) | Slow memory growth on long-lived instances | **Resolved 2026-09-12 (pass 6)** — expired-bucket sweep when map exceeds 10k; pinned by 2 unit tests (eviction + active-window preservation) |
| **LOW** | ~~Markdown link hrefs accepted any scheme~~ (audit A-02) | `javascript:`/`data:` URLs in content would render as anchors (defense-in-depth; content is dev-authored) | **Resolved 2026-09-12 (pass 6)** — `SAFE_HREF` allow-list (http/https/mailto/root-relative); unsafe schemes degrade to plain text; 2 unit tests |
| **LOW** | ~~FAQ `<details>` groups not exclusive~~ (audit A-04) | Minor UX divergence from the source's Radix accordion | **Resolved 2026-09-12 (pass 6)** — `name="home-faq"/"guide-faq"/"calc-faq"` exclusive accordions (progressive enhancement); E2E pin |
| **INFO** | Audit false-positives + dev-only advisories (A-03 JSON-LD `dangerouslySetInnerHTML` static-safe; A-05 npm audit 6 dev-only moderates — vitest mocker + esbuild via drizzle-kit, 0 prod vulns; A-06 probe scripts) | None at runtime | **Accepted + documented** in `docs/AUDIT_REPORT_pass5.md` — re-evaluate on next toolchain bump |
...
No `CRITICAL` open issues. `lint 0/0`, `typecheck` pass, `41/41` unit (11+13+9+8), `121/121` E2E per project (120/121 DB-less), `build 43/43` (35 static + 8 dynamic), live probes green as of `2026-09-15` (pass 9 validation `docs/VALIDATION_REPORT_2026-09-15.md` 121/121; pass 4: source-exact home hero, hub truth callout + Last-Updated, loan-page source heroes + author strips, learn read-duplication fix, calculator pill + blue tax segment, intro-eyebrow removal — see `docs/REMEDIATION_PLAN_pass4.md`).
...
| `src/app/page.tsx` | ~500 | `/` editorial homepage (hero, wordmarks, intro eyebrow + cards 4, problem/fix, steps 3, testimonials, FAQ, closing CTA + trust line — pass 3 removed the green band) |
| `src/app/sitemap.ts` | ~70 | `STATIC_PATHS 39` + `articles→/learn/[slug]` (23) + `states` (50) + `manufacturers` (40) → 152 locs (39+23+50+40) |
| `src/components/ui.tsx` | ~80 | `cn`, `Button/ButtonLink` (6 variants, 3 sizes), `Container (1400px)`, `Badge` |
| `src/components/site-header.tsx` | ~120 | `"use client"` fixed `h-16`, always-light frosted bar (pass 3), `NAV` 4 + `MORE` 2, `useId`, adjust-during-render pathname close |
...
| `vitest.config.ts` | ~15 | Node env, alias `@→./src`, `include src/**/*.test.ts` (4 files, 41 tests = 11+13+9+8) |
...
| `e2e/{smoke,seo,funnel,assets,parity}.spec.ts` | 63/77/99/74/956 | 121 E2E tests per project (smoke 8, seo 16, funnel 5, assets 19 runtime, parity 73 runtime; 103 declarations + data-driven loops — `playwright --list` shows 122 listed but 121 executed) incl. H4-OOM + alias/image regression + pass-5/7 live-source parity guards |
| `src/lib/*.test.ts` | 81/110/57/146 | 41 Vitest unit tests co-located on pure domains (calculator 11 + matching 13 + rate-limit 9 + markdown 8) |
...
*Last verified 2026-09-15 (PAD v1.6 — pass 9 drift alignment D-01→D-07: STATIC_PATHS 40→39, build 36+7→35+8, E2E 82→121, vitest 3.2→4.1, playwright 122→121, host locked; validation `docs/VALIDATION_REPORT_2026-09-15.md` GO; remediation pass 4: source-exact home hero (daylight photo + `from-primary/95 via-primary/90 to-primary/80` + bottom fade + content-driven height + text-4xl→6xl headline, no hero-grid), intro eyebrow REMOVED (live source renders none — pass-3 pin flipped), hub hero Last-Updated line + "Here's the truth" glass callout + dual CTA + 4th chip + crumbsOutside breadcrumbs, FHA/VA/USDA/construction loan pages source copy + amber-first CTA + contextual eyebrow icons + author/reviewer strips + FHA chip removal, learn "read read" fix, calculator Free-Calculator pill + middle crumb + `--color-chart-tax` blue tax segment, header blur 12→16px, hero photo + skyline wordmark swapped to source assets; pass 3 (2026-09-12): circle brand mark + two-tone wordmark, always-light header, closing trust line, green band removed, get-started 3-step band, calculator amber band, footer legal line + 2-item More dropdown, PMI-flake root cause; counts `41/41` vitest = 11+13+9+8 + `121/121` per project playwright = smoke 8+seo 16+funnel 5+assets 19+parity 73 (120/121 DB-less; 122 listed→121 executed) + `next build` 43/43 (35 static + 8 dynamic) + `lint 0/0` + live probes (`/api/health` ok:true, funnel 4 matches, sitemap 152 = 39+23+50+40, robots host correct) against `package.json` (next ^16.3.4, react ^19.3.0, tailwind ^4.3.3, vitest ^4.1, playwright 1.63), `tsconfig.json` + `eslint.config.mjs` (skills+infrastructure ignored), `next.config.ts` (11 redirects), `drizzle.config.ts/.json` (5434, strict/verbose), `docker-compose.yml` (home_financing_*), `src/db/schema.ts` (8 tables), `src/lib/*` (41 tests = 11+13+9+8), `public/brand/wordmarks 5` + `public/images/avatars 3` + `public/images 5` + `brand/og-image.jpg`, `playwright.config.ts` (3002), `e2e/*` 121 per project (103 decl; 122 listed), `vitest` 41/41. Source of truth: `package.json`, `tsconfig.json`, `next.config.ts`, `eslint.config.mjs`, `drizzle.config.ts/.json`, `docker-compose.yml:5434`, `src/db`, `src/lib`, `src/scripts`, `e2e`, `vitest.config.ts`, `.env.example`, live modfii.com + modfii.jesspete.shop probes. Evidence: `docs/REMEDIATION_PLAN_pass4.md` (pass 3: `docs/REMEDIATION_PLAN_pass3.md`).*
