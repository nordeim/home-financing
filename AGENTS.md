...
| `npm run e2e` | Playwright E2E chromium (prod `next start` on 3002, 55 tests per project — 42 declarations + 14 asset-title variants; 54/55 without a DB) |
...
- **Visual parity with modfii.com is a design contract.** The header is a light frosted bar (`bg-background/90` + blur) in every state — live modfii.com never goes transparent over the hero (computed-style probe 2026-09-12; the earlier transparent-over-dark treatment was removed in pass 3). Interior heroes flow through `PageHero` (`src/components/page-shell.tsx`) — centered, photo-backed, star eyebrow pill, optional amber `highlight` title line, CTA pair, glass stat chips. Don't reintroduce left-aligned gradient heroes or an amber header CTA (it is a forest "Get Started" pill).
...
- **Tests: Vitest unit + Playwright E2E.** 37 unit tests in `src/lib/*.test.ts` (`npm run test`: calculator 11 + matching 13 + rate-limit 7 + markdown 6) + 55 E2E tests per project in `e2e/` (`smoke 7`/`seo 6`/`funnel 4`/`assets 19 runtime`/`parity 19`, chromium via `npm run e2e`; 42 declarations + 14 asset-title variants). `assets.spec.ts` guards fourteen image assets (6 heroes/OG + 5 manufacturer wordmarks + 3 testimonial avatars) + two `/compare/*` aliases (2026-09-11 incidents). `parity.spec.ts` pins the markdown-OOM regression (`#### ` articles must render), the modfii.com visual-parity additions, and the pass-3 live-source parity pins (logo two-tone/circle mark, always-light header, intro eyebrow, closing trust line, no green band, get-started 3-step band, calculator amber band, footer legal line, 2-item dropdown). The funnel valid-payload E2E needs Postgres; the rest run DB-less. The calculator-PMI interaction test navigates fresh per attempt (`toPass`) — a pre-hydration synthetic dispatch poisons React's value tracker and identical re-dispatches are swallowed (2026-09-12 flake, root-caused).
...
*Last verified 2026-09-13 (remediation pass 3: live-source parity — circle brand mark + two-tone wordmark, always-light header, homepage intro eyebrow + closing trust line, green-band and duplicate-steps-CTA removal, get-started 3-step band, calculator amber band, footer legal line + 2-item dropdown; PMI hydration flake root-caused and pinned with fresh-navigation retry; docs normalization: 37 unit = 11+13+7+6, 55 E2E per project = 42 declarations + 14 asset variants; error boundary `retry` prop aligned to Next 16.3 stable) against `package.json` (next ^16.3.4, react ^19.3.0, tailwind ^4.3.3, vitest ^3.2), `eslint.config.mjs` (skills + infrastructure ignored), `tsconfig.json`, `next.config.ts` (11 redirects), `drizzle.config.ts/json` (5434), `docker-compose.yml`, `src/db` (8 tables), `src/lib` (calculator 11 + matching 13 + rate-limit 7 + markdown 6), `public/images/*` + `public/brand/*` (every referenced asset exists), `playwright.config.ts` (3002), `e2e/*` 55 tests per project, `vitest.config.ts`. Live-source evidence: `docs/REMEDIATION_PLAN_pass3.md`.*
