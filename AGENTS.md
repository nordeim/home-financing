...
| `npm run e2e` | Playwright E2E chromium (prod `next start` on 3002, 81 tests per project; 80/81 without a DB) |
...
- **Tests: Vitest unit + Playwright E2E.** 37 unit tests in `src/lib/*.test.ts` (`npm run test`: calculator 11 + matching 13 + rate-limit 7 + markdown 6) + **81 E2E tests per project** in `e2e/` (`smoke 8`/`seo 5`/`funnel 5`/`assets 19 runtime`/`parity 44 runtime`, chromium via `npm run e2e`; 52 declarations + data-driven loops). `assets.spec.ts` guards fourteen image assets + two `/compare/*` aliases (2026-09-11 incidents). `parity.spec.ts` pins the markdown-OOM regression, the visual-parity additions, the pass-3/4 live-source pins, and the pass-5 pins (rotated-square badge, header bar metrics /80+16px+h-20, hero source paddings + `max-w-xl` grid, intro eyebrow RESTORED — the pass-4 removal was a wrong pin, button chrome 10px/500/px-8, radius tokens md=10px/2xl=16px, wordmark strip `py-8 bg-muted/30 border-y`, testimonial `bg-background` 16px cards, heading hierarchy hero-H2/intro-H1, FAQ H3 questions, interior H1 sizes + long-form guide titles, SEO title patterns, guide content-depth outlines for hub/FHA/ADU/tiny/construction, `/debug-error-probe` error-boundary render). The funnel valid-payload E2E needs Postgres; the rest run DB-less (80/81). Security headers are pinned by `smoke.spec.ts` (CSP/XFO/nosniff/referrer/permissions from `next.config.ts:headers()`), and the JSON-error contract (incl. DB-outage 500) by `funnel.spec.ts` + `scripts/verify-db-outage.sh`.
